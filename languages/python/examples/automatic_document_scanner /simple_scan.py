...
def scan(img):
    ...
    # for point in destination_corners:
    #     cv2.circle(orig_img, (int(point[0]), int(point[1])), 5, (0, 0, 255), -1)
    # return orig_img


    h, w = orig_img.shape[:2]
    print('>>', destination_corners, corners, w, h)
    # Getting the homography.
    homography, mask = cv2.findHomography(np.float32(corners), np.float32(destination_corners), method=cv2.RANSAC,
                                          ransacReprojThreshold=3.0)
    print(np.float32(corners))
...
for img_path in glob.glob('inputs/4.jpg'):
    try:
